...
const s = {
  page: { minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f1f5f9', padding: '20px 0' },
...
export default function Register() {
  ...
  const handleVerifyAndActivate = async () => {
    ...
    try {
      ...
      await API.post(
        '/auth/2fa/enable',
        { token: twoFA.token },
        { headers: { Authorization: `Bearer ${twoFA.tempToken}` } }
      );
  ...
  if (twoFA.show) return (
    <div style={s.page}>
      <div style={s.card}>
        <div style={s.title}>📱 Security Setup</div>
        <div style={s.sub}>Scan this QR code with your authenticator app</div>
  ...
  return (
    <div style={s.page}>
      <div style={s.card}>
        ...
        <div style={s.sub}>Join SecureVault document storage system</div>
        {error && <div style={s.err}>{error}</div>}
